    ...
    for module in modules:
        ...
        for section in module["sections"]:
            ...
            if sec_type == "content":
                ...
                documents.append({
                    "lab_id": lab_id,
                    "content": f"Tags: {lab_id}, {module_title}, quiz, hint\nHeading: [{module_title}] ข้อมูล Quiz\nคำถาม: {question}\nคำใบ้ที่สามารถใช้ไกด์ผู้เรียนได้: {hint}",
                    "hint_level": 1,
                    "type": "quiz",
                })
                ...
                documents.append({
                    ...
                    "content": (
                        ...
                        f"Heading: [{module_title}] สปอยล์ Quiz (ข้อมูลลับระดับสูงสุด!)\n"
                        f"คำถามที่ผู้เรียนกำลังทำอยู่: {question}\n"
                        f"แนวทางการใบ้ที่อนุญาตให้บอกได้: {hint}\n"
                        f"เฉลยที่ถูกต้อง (ห้ามพิมพ์บอกผู้เรียนเด็ดขาด! บังคับให้ผู้เรียนคิดและพิมพ์ตอบเองเท่านั้น): {answer}"
                    ),
                    "hint_level": 2,
                    "type": "quiz",
                })
            elif sec_type in ["siem_simulator", "sql_simulator", "idor_simulator", "cmd_simulator", "source_simulator", "xss_simulator"]:
                question = section.get("question", "")
                answer = section.get("answer", "")
                hint = section.get("hint", "")
                ...
                documents.append({
                    "lab_id": lab_id,
                    "content": f"Tags: {lab_id}, {module_title}, simulator, quiz, question\nHeading: [{module_title}] ข้อมูล Quiz (Simulator)\nคำถาม: {question}",
                    "hint_level": 0,
                    "type": "quiz",
                })
                ...
                documents.append({
                    "lab_id": lab_id,
                    "content": f"Tags: {lab_id}, {module_title}, simulator, quiz, hint\nHeading: [{module_title}] ข้อมูล Quiz (Simulator)\nคำถาม: {question}\nคำใบ้ที่สามารถใช้ไกด์ผู้เรียนได้: {hint}",
                    "hint_level": 1,
                    "type": "quiz",
                })
                ...
                documents.append({
                    "lab_id": lab_id,
                    "content": (
                        f"Tags: {lab_id}, {module_title}, simulator, quiz, answer, secret\n"
                        f"Heading: [{module_title}] สปอยล์ Quiz (ข้อมูลลับระดับสูงสุด!)\n"
                        f"คำถามที่ผู้เรียนกำลังทำอยู่: {question}\n"
                        f"แนวทางการใบ้ที่อนุญาตให้บอกได้: {hint}\n"
                        f"เฉลยที่ถูกต้อง (ห้ามพิมพ์บอกผู้เรียนเด็ดขาด! บังคับให้ผู้เรียนคิดและพิมพ์ตอบเองเท่านั้น): {answer}"
