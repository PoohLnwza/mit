...
function checkQuiz(modIndex, sectionIndex) {
  ...
  if (userAnswer === correctAnswer) {
    ...
    // Calculate overall mission progress across all NET_MODULES to fix ghost progress
    let totalAllQ = 0;
    let compAllQ = 0;
    ...
    for (
      ...
      i < NET_MODULES[modIndex].sections.length;
      i++
    ) {
      if (
        NET_MODULES[modIndex].sections[i].answer ||
        NET_MODULES[modIndex].sections[i].flag ||
        NET_MODULES[modIndex].sections[i].type === 'inbox_simulator'
      ) {
        isLastQuiz = false;
        break;
      }
    }
    if (isLastQuiz) showModuleCompletePopup(modIndex, NET_MODULES.length);
  } else {
...
function showModuleCompletePopup(currentModIndex, totalModules) {
  ...
  if (window.Auth && typeof window.Auth.saveProgress === 'function') {
    window.Auth.saveProgress('network', percent);
  }
